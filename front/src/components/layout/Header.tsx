  ...
  return (
    <header className="bg-[#1d3557] h-14 flex items-center px-6 fixed top-0 left-0 right-0 z-40 shadow-md">
      ...
      <div className="flex items-center gap-2 relative" ref={menuRef}>
        ...
        {!admin && (
          <button onClick={toggleAlerts} className="relative p-2 hover:bg-white/10 rounded-full transition-colors" aria-label={notifications.unreadCount > 0 ? `알림 ${notifications.unreadCount}건` : '알림'}>
            ...
            {notifications.unreadCount > 0 && (
              <span className="absolute top-0.5 right-0.5 min-w-4 h-4 px-1 bg-red-500 text-white text-[10px] font-bold leading-4 text-center rounded-full border border-[#1d3557]">

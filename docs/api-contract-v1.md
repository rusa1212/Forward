...
| `counts.newToday` | 그중 오늘 수집된 건수 — **⚠️ 7-4 버그 참고** |
...
### 7-2. `GET /dashboard/trend`

대시보드 "매칭 추이" 위젯용 — 올해·작년의 **월별(1~12월) 매칭 건수**. 파라미터 없음.
2026-09-11 이전에는 FE(`TrendChart.tsx`)가 하드코딩된 샘플 배열을 그렸는데, 실 데이터로 교체하며 이 엔드포인트를 새로 만들었다.
...
```json
{
  ...
  "data": {
    "months": ["1월", "2월", "...", "12월"],
    "series": [
      { "year": 2025, "counts": [4, 6, 5, 8, 7, 9, 8, 10, 9, 11, 10, 12] },
      { "year": 2026, "counts": [7, 9, 8, 12, 11, 14, 13, 16, 0, 0, 0, 0] }
    ]
  }
}
```
...
| 필드 | 의미 |
| --- | --- |
| `months` | 항상 `["1월", ..., "12월"]` (길이 12, 고정) |
| `series` | **작년, 올해 순으로 정확히 2개.** 각 `counts`는 길이 12 — 아직 안 지난 달(예: 올해 9월 기준 10~12월)은 `0` |

- 매칭 기준은 7-1과 동일(**키워드별 제목 부분일치의 OR**). 등록 키워드가 0개면 두 `counts` 모두 전부 `0`이다.
- 집계 기준은 공고가 "몇 월에 수집됐는지"(`collected_at`, KST 변환 후 연/월 추출). 발표 마감일(`reception_end`) 기준이 아니다.
- FE는 `series[1]`(올해)의 **이번 달** 값에 점을 찍어 툴팁을 띄운다. 배열 마지막(12월) 인덱스가 아니라 `new Date().getMonth()` 기준이다 — 안 지난 달은 `0`으로 채워져 있어서 마지막 인덱스를 쓰면 항상 12월(대부분 0건)을 가리키게 된다.
...
- **`trend`(매칭 추이)는 `summary`와 별도 API라 `TrendChart`가 자체적으로 로딩/에러 상태를 갖고 따로 호출한다.** 위젯이 숨겨져 있으면(위젯 편집) 애초에 렌더링되지 않아 호출도 안 나간다.

### 7-4. ⚠️ `newToday`에 타임존 버그가 있다 (BE 수정 필요)
...
| 🔴 BE 수정 | `dashboard/summary`의 `newToday` 타임존 버그 (7-4절) |
